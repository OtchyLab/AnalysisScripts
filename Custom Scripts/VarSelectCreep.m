function VarSelectCreep(end_date, earliest_date, interval, test_syls)
%
...
parentdest = 'C:\Documents and Settings\Bird Brain\Desktop\Rd278 SortedTEMP';
...
while datenum(date, 'yyyy-mm-dd') >= datenum(earliest_date, 'yyyy-mm-dd')
    ...
    if isdir(songsource)
        ...
        mem_index = [1:1000:length(cur_DataSet), length(cur_DataSet)];
        simfeat = [];
        for q = 1:(length(mem_index)-1)
            if q == (length(mem_index)-1)
                TempSet = [TestFeatures cur_DataSet(:,mem_index(q):(mem_index(q+1)))];
            else
                TempSet = [TestFeatures cur_DataSet(:,mem_index(q):(mem_index(q+1)-1))];
            end
            featdist = squareform(pdist(TempSet'));
            if q == 1
                simfeat = [simfeat featdist(1:size(TestFeatures,2),:)];
            else
                simfeat = [simfeat featdist(1:size(TestFeatures,2),size(TestFeatures,2)+1:end)];
            end
        end
        %jndDataSet = [TestFeatures eval(['DataSet_' datestr(datenum(date, 'yyyy-mm-dd'), 'yyyy_mm_dd')])];
        %simfeat=squareform(pdist(jndDataSet'));
        %***************
        dist_th = .25;   %This is the distance in feature space within which the algorithm will creep.
        %***************
        if isdir(dest)
        else
            mkdir(dest);
...
%             testnum = min(length(index), 150);
            ...
            while added~=0
                if run_at_all == 0
                    index = find(simfeat(k,length(test_syls)+1:end) <= dist_th);
                    SylBank = index;
                    run_at_all = 1;
                    added = length(index);
                else
                    notchecked = setdiff(SylBank, checked);
                    mem_index = [1:500:length(cur_DataSet), length(cur_DataSet)];
                    for j = 1:length(notchecked)   
                        for q = 1:(length(mem_index)-1)
                            if q == (length(mem_index)-1)
                                Temp = [cur_DataSet(:,notchecked(j)) cur_DataSet(:,mem_index(q):(mem_index(q+1)))];
                            else
                                Temp = [cur_DataSet(:,notchecked(j)) cur_DataSet(:,mem_index(q):(mem_index(q+1)-1))];
                            end
                            featdist = squareform(pdist(Temp'));
                            if q == 1
                                creepfeat = [creepfeat featdist(1,:)];
                            else
                                creepfeat = [creepfeat featdist(1,2:end)];
                            end
                        end
                    creepindex = (find(creepfeat(1,2:end) <= dist_th))-1;
                    creepfeat = [];
                    SylBank = unique([SylBank creepindex]);
                    creeptotal = unique([creeptotal creepindex]);
                    run_at_all = 1;
                    end
                checked = unique([checked notchecked]);
                added = length(creeptotal);
                creepindex = []; creeptotal = [];
                end
            end   
