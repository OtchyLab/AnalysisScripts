function inactSumStatsPlotV2(inactStats)
...
% saveName = 'Summary Inactivation Data Intermediate 05152015';
...
eb = errorbar(1:5, [mean(ddDist), mean(ppDist), mean(inactDist), mean(PBSDist), mean(elevDist)], [std(ddDist), std(ppDist), std(inactDist), std(PBSDist), std(elevDist)]./sqrt(5),'sk');
xlim([0.5,5.5]); ylim([0,75]); %axis square
ylabel('Norm EMD (AU)', 'FontSize', 12)
title(['Earth Mover Distance'], 'FontSize', 12);
set(gca, 'Box', 'off', 'TickDir', 'out', 'XTick', [1, 2, 3, 4, 5], 'XTickLabel', [{'Cntl'}, {'P-P'}, conditions], 'YTick', [0:25:75], 'LineWidth', 2, 'FontSize', 10)   

for i = 1:5
    
    set(q(i), 'Marker', 'o', 'Color', [0.5, 0.5, 0.5], 'MarkerSize', 10, 'LineStyle', 'none');
    set(eb, 'Marker', 's', 'Color', 'k', 'MarkerSize', 6, 'LineStyle', 'none', 'LineWidth', 1.5);
end

set(gcf, 'Units', 'Inches', 'Position', [0 0 4 3])

%Hypothesis Testing
[testOut.ddpp,pOut.ddpp,ci.ddpp,~] = ttest2([ddDist],[ppDist]);
[testOut.ddInact,pOut.ddInact,ci.ddInact,~] = ttest2([ddDist],[inactDist]);
[testOut.ddPBS,pOut.ddPBS,ci.ddPBS,~] = ttest2([ddDist],[PBSDist]);
[testOut.ddElev,pOut.ddElev,ci.ddElev,~] = ttest2([ddDist],[elevDist]);

%Save figure and data
saveFolder = 'C:\Users\Tim\Desktop\Nif Project Figures\Inactivations\';
saveName = 'Summary Inactivation Data Final 05162015';
