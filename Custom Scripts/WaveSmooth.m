...
% Smooth and plot sound amplitude
%handles.smoothamp = smooth(handles.amplitude,50,'rlowess');
handles.smoothamp = smoothn(handles.amplitude,[21;21],'box');
subplot(5,1,3);
plot((0:length(handles.wav)-1)/handles.fs,handles.smoothamp,'r');
hold on
handles.thresline = plot([0 (length(handles.wav)-1)/handles.fs],repmat(threshold,1,2),':r');
hold off
box off
...
% Plot sound and smooth on one plot
subplot(5,1,4);
plot((0:length(handles.wav)-1)/handles.fs,handles.amplitude,'b');
hold on
plot((0:length(handles.wav)-1)/handles.fs,handles.smoothamp,'r');
