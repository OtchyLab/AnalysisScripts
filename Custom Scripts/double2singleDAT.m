function [filename, DAT] = double2singleDAT(fullFilename)
%This function takes as input the full filename (i.e., absolute path +
%filename) for a 5-channel DAT file that was wrongly encoded with double (instead of single) precision chunks
%and convertas it to the standard 5-channel DAT structure.
...
for i=1:numChan
   chunks(i,:) =  fread(fid, fs,'double',0,'b');
end
