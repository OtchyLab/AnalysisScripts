function [mu_SF, sig_SF, prune] = genRAparams(age, paradigm)
% function [mu_ISI, sig_ISI, mu_numBurst, sig_numBurst] = genRAparams(age)
% Given a specified bird age, this function generates physiologically realistic RA input
% parameters based on JGO's in vitro measurements
%
...
if strcmp(paradigm, 'tutored') %Tutored birds
    %Single fiber strength
    sf_m = [50, 75, 80]./1000;
    sf_s  = [30, 60, 44]./1000;
    
    %Estimated number of inputs
    sp_rawPrun = [26.4, 10.8, 10];
    
elseif strcmp(paradigm, 'isolated') %Isolate birds
    %Single fiber strength
    sf_m = [45.4, 73, 67]./1000;
    sf_s  = [30.4, 72.8, 57.1]./1000;
    ...
    %That's a problem... something is fucked
    display('Something went wrong with the paradigm input... check and try again.')
    beep
    return
end
...
m_SF = interp1(agePoints, sf_m, age);
s_SF = interp1(agePoints, sf_s, age);
var_SF = s_SF^2;
prune = interp1(agePoints, sp_Prun, age);

%Generate lognormal mu and sigma from the HVC descriptive statistics
mu_SF = 2 * log(m_SF) - 0.5 * log(var_SF + m_SF^2);
sig_SF = sqrt(log(var_SF + m_SF^2) - 2 * log(m_SF));
