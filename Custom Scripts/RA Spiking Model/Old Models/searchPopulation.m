...
% searchPopulation
% written by TMO 06/11/2016; last modified 06/17/2016
%
...
%inh_HVC_ratio = 200;
inh_HVC_ratio = linspace(300, 3000, 55);
...
for i = 1:numel(numHVC)
    ...
    for j = 1:numel(inh_HVC_ratio)
        ...
        D(i,j) = calcDistance(D, summary(i,j), targetStats, targetRng, simID, numHVC(i), inh_HVC_ratio(j));
...
saveFolder = 'C:\Users\Tim\Desktop\RA Model Outputs\';
dati = strrep(char(datetime), ':', '-');
    if i == 1
        %Maturation Vectors
        figName = ['PopSearch - ' simID ' - ' dati ' - Maturation Input Vectors.fig'];
        savefig(f0, [saveFolder, figName])
        
        %Target Vectors
        figName = ['PopSearch - ' simID ' - ' dati ' - Target Vectors.fig'];
        savefig(f1, [saveFolder, figName])
    end
    
    %Save the current spiketrain stats figure
    figName = ['PopSearch - ' simID ' - ' dati ' - SpikeTrain Stats numHVC=' num2str(numHVC(i)) '.fig'];
...
function D = calcDistance(Din, summary, targetStats, targetRng, simID, numHVC, inh_HVC)
...
if plotIt
    ...
    set(gcf, 'Units', 'inches', 'Position', [2, 2, 5, 9])
    drawnow
end
