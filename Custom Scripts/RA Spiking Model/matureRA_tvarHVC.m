function [spikeStats, params] = matureRAtvarHVC(numHVC, inh_HVC_ratio, matureVecs)
% [spikeStats, params] = matureRA(numHVC, inh_HVC_ratio, matureVecs)
% written by TMO 06/11/2016; last modified 06/30/2016
%
...
%                 
%                 nodeSpace = 10;
%                 HVC_spikes = genHVCinput(W_HVC, numSpikes, mu_ISI, sig_ISI, dt, nodeSpace);
%                 
%                     %Retrive the correct number of spikes and ISIs as defined by distributions
%                     %numSpikes = max([1, round(normrnd(mu_numSp, sig_numSp, 1))]);        %normal distribution
%                     ISIs = lognrnd(mu_ISI, sig_ISI, 1, numSpikes(k));      %lognormal distribution
...
%             end
            ...
            %Generate noisy HVC spiking input
            nodeSpace = 10; % spacing between HVC nodes (ms); 
            HVC_spikes = genHVCinput(W_HVC, numSpikes, mu_ISI, sig_ISI, dt, nodeSpace);
...
%     [spike_times, Vm] = RA_spiking_neuron(HVC_spikes, inh_HVC, numTrials, t, numHVC, numLMAN);
    ...
    if raster && ((i==1) || ages(i)==90 || (i == (numel(ages))))
...
%         end
        if i == 1
            ...
            subplot(3, 1, 1);
            age = '60';
            title(['Example neuron - ' num2str(numHVC), ' - ', num2str(inh_HVC_ratio)])
        elseif ages(i) == 90
            %Plot the first transition 'day' of the simulation in subplot 2
            subplot(3, 1, 2);
            age = '90';
        ...
        if i == numel(ages)
           ...
           savefig(q, ['C:\Users\Tim\Desktop\RA Model Example figures\Example neuron - ' num2str(numHVC), ' - ', num2str(inh_HVC_ratio), ' - ', simID, '.fig']);
        end
