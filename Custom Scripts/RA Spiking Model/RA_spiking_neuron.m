function [spike_times, Vm] = RA_spiking_neuron(HVC_spikes_array, inh_HVC, numTrials, t, numHVC, numLMAN)
...
%
% Inputs:
% HVC_spikes:   either: an array of length(t_stop/dt); contains the weighted inputs from HVC
%               or: a matrix of size numTrials x length(t_stop/dt); contains the weighted inputs from HVC for each trial
...
elseif nargin == 2
    ...
    numLMAN = 1;
    inh_HVC = 10;
    display('Using the default parameters.')
...
%Check to see if the HVC spiking data is trial-by-trial
if size(HVC_spikes_array,1)>1
    trialSpikes = true;
else
    trialSpikes = false;
end
...
for i = 1 : numTrials
    ...
    %Extract this rendition's HVC spike times
    if trialSpikes
        HVC_spikes = HVC_spikes_array(i,:);
    else
        HVC_spikes = HVC_spikes_array;
    end
