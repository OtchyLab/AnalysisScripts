...
% searchPopulation
% written by TMO 06/11/2016; last modified 10/10/2017
%                            most recent mod to remove the subsong timepoints
%
...
[prune_vec, mean_vec, std_vec, ages, inh_vec, HVC_vec, HVCisi_m_vec, HVCisi_s_vec, HVCnum_m_vec, HVCnum_s_vec] = expVects;
...
matureVecs.HVCisi_m_vec = HVCisi_m_vec;
matureVecs.HVCisi_s_vec = HVCisi_s_vec;
matureVecs.HVCnum_m_vec = HVCnum_m_vec;
matureVecs.HVCnum_s_vec = HVCnum_s_vec;
...
subplot(3,1,3)
[hAx, hLine1, hLine2] = plotyy(ages, HVCnum_m_vec, ages, HVCisi_m_vec);
xlabel('Age (d)')
ylabel(hAx(1), 'Mean HVC Spikes')
ylabel(hAx(2), 'Mean HVC ISI (ms)')
xlim(hAx(1), [35, 210]); ylim(hAx(1), [2, 6])
xlim(hAx(2), [35, 210]); ylim(hAx(2), [2, 6])
set(hAx(1), 'Box', 'off', 'TickDir', 'out', 'YTick', 2:2:6)
set(hAx(2), 'Box', 'off', 'TickDir', 'out', 'YTick', 2:2:6)

set(gcf, 'Units', 'inches', 'Position', [1, 1, 4, 7])
drawnow
...
%numHVC = 4;
numHVC = linspace(13.5, 14.5, 11);

%Specify inh_HVC_ratio to search
%inh_HVC_ratio = 200;
inh_HVC_ratio = linspace(3300, 3500, 11);
...
%             [s, p] = matureRA(numHVC(i), inh_HVC_ratio(j), matureVecs);
            [s, p] = matureRA_tvarHVC(numHVC(i), inh_HVC_ratio(j), matureVecs);
        ...
        [D(i,j), hq] = calcDistance(D, summary(i,j), targetStats, targetRng, simID, numHVC(i), inh_HVC_ratio(j));
        ...
        set(gca, 'Box', 'off', 'TickDir', 'out', 'FontSize', 11, 'LineWidth', 1.5)
...
%     saveFolder = '/Users/Tim/Desktop/Iso Paper Datasets/Modeling Figures/';
    saveFolder = 'C:\Users\Tim\Desktop\RA Model\';
    if i == 1
        %Maturation Vectors
        figName = ['PopSearch - ' simID ' - ' char(datetime('now','Format','d-MMM-y HH-mm-ss')) ' - Maturation Input Vectors.fig'];
        savefig(f0, [saveFolder, figName])
        
        %Target Vectors
        figName = ['PopSearch - ' simID ' - ' char(datetime('now','Format','d-MMM-y HH-mm-ss')) ' - Target Vectors.fig'];
        savefig(f1, [saveFolder, figName])
    end
    
    %Save the current spiketrain stats figure
    figName = ['PopSearch - ' simID ' - ' char(datetime('now','Format','d-MMM-y HH-mm-ss')) ' - SpikeTrain Stats numHVC=' num2str(numHVC(i)) '.fig'];
    ...
    %Save the best match plot
    figName = ['PopSearch - ' simID ' - Best Match.fig'];
    savefig(hq, [saveFolder, figName])
...
%HVC characteristics
sp_HVC = [4, 2, 2]; %age-dependent decrease in HVC ISI (ms)
sp_HVCisi_m = [mean([4.2707, 3.0033]), 1.63, 1.63]; %estimated from Okubo et al and Hahnloser et al 2002
sp_HVCisi_s = [mean([1.9258, 1.7302])  4.76, 4.76]; %estimated from Okubo et al and Hahnloser et al 2002

sp_HVCnum_m = [mean([4.7, 3.8]), 4.5, 4.5]; %estimated from Okubo et al and Hahnloser et al 2002
sp_HVCnum_s = [mean([2.2, 1.3]), 2, 2]; %estimated from Okubo et al and Hahnloser et al 2002

%Inhibition scaling
%sp_inh = [1, 1, 1]; % age-dependent inhibition based on Olveczky 2011
sp_inh = [1, 0.60, 0.60]; % age-dependent inhibition based on Sakaguchi 1996

%Tutored
sp_Mean = [50, 75, 80]./1000;
sp_Std  = [30, 60, 44]./1000;
sp_rawPrun = [26.4, 10.8, 10];
...
%Interpolate between points
r2 = linspace(sp_ages(1), sp_ages(2), s2); r3 = linspace(sp_ages(2), sp_ages(3), s3);
ages = [r2, r3(2:end)];

mean_vec = interp1(sp_ages, sp_Mean, ages);
std_vec = interp1(sp_ages, sp_Std, ages);
prune_vec = interp1(sp_ages, sp_Prun, ages);
inh_vec = interp1(sp_ages, sp_inh, ages);
HVC_vec = interp1(sp_ages, sp_HVC, ages);
HVCisi_m_vec = interp1(sp_ages, sp_HVCisi_m, ages);
HVCisi_s_vec = interp1(sp_ages, sp_HVCisi_s, ages);
HVCnum_m_vec = interp1(sp_ages, sp_HVCnum_m, ages);
HVCnum_s_vec = interp1(sp_ages, sp_HVCnum_s, ages);
...
if tutored
    ...
    FR = [47, 75.9, 75.7];
    BF = [0.68, 0.84, 0.97];
    sparse = [0.16, 0.20, 0.23];
    spikeCorr = [0.69, 0.80, 0.85];
else
    %Isolate stats
...
function [D, hq] = calcDistance(Din, summary, targetStats, targetRng, simID, numHVC, inh_HVC)
...
% %No scaling
% simStats = [];
% simStats(1,:) = summary.FR;
% simStats(2,:) = summary.BF;
% simStats(3,:) = summary.sparse;
% simStats(4,:) = summary.spikeCorr;
% simStats(isnan(simStats)) = 0;
...
if D < min(Dtemp)
    ...
    xlim(xlims); ylim([0,1])
    ylabel('Firing Rate')
    title([simID ' Best Match Found - numHVC: ' num2str(numHVC) ' inhHVC: ' num2str(inh_HVC)])
    set(gca, 'Box', 'off', 'TickDir', 'out')
...
%For simulated data, don't lose the upper and lower bound limits
if nargin == 3
    lowIdx = in < rnge(1);
    highIdx = in > rnge(2);
    out(lowIdx) = 0;
