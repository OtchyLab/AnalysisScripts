function VarSelect(end_date, earliest_date, interval, test_syls)
%
...
parentdest = 'C:\Documents and Settings\Bird Brain\Desktop\Rd278 Sorted M 1.4A';
...
while datenum(date, 'yyyy-mm-dd') >= datenum(earliest_date, 'yyyy-mm-dd')
    ...
    if isdir(songsource)
        ...
        for q = 1:(length(mem_index)-1)
            ...
            featdist = squareform(pdist(TempSet','mahalanobis'));
            if q == 1
        ...
        %***************
        dist_th = 1.4;   %This is the distance in feature space within which 2 syllables are "the same"; needs improvement.
        %***************
        if isdir(dest)
        else
            mkdir(dest);
