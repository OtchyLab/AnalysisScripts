function motifShifts = crossCorrelate(startMotif, endMotif, spikeTimes, sampRate, bInstant, bRotate, gaussWidth, startTime,endTime, sampRateOut)
%Aaron Andalman 2004
...
for(nMotif = startMotif:endMotif)
    ...
    %if(length(find(spikeTime >= startTime & spikeTime <= endTime)))      
        ...
        if(exist('gaussWidth') & gaussWidth > 0)
            ...
            sig = resample(sig,500,sampRate); %resample to 500 Hz sampling
        end
